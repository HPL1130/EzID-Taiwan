...
const EzIDApp = () => {
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-gray-50 min-h-screen font-sans text-sm">
      <h1 className="text-center font-black text-blue-600 text-xl mb-4 tracking-tighter">EzID 台灣證件照</h1>
      ...
      {!image ? (
        <label className="block border-2 border-dashed border-gray-300 rounded-[2.5rem] p-16 text-center cursor-pointer bg-white">
          ...
          {photoList.length > 0 && <button onClick={(e) => {e.preventDefault(); downloadPrint();}} className="mt-4 block w-full bg-blue-600 text-white py-3 rounded-xl">下載拼板</button>}
        </label>
      ) : (
            ...
            <div className="border-t pt-3">
              ...
              <div className="flex gap-2 overflow-x-auto py-1 no-scrollbar">
                ...
                {CLOTHES_DATA[gender].map(s => (
                  <button key={s.id} onClick={() => setSelectedSuit(s)} className={`w-12 h-12 border-2 rounded flex-shrink-0 overflow-hidden ${selectedSuit?.id === s.id ? 'border-blue-500' : ''}`}>
              ...
              {selectedSuit && (
                <div className="flex gap-1 mt-2">
                  <button onClick={() => setSuitConfig(p => ({...p, y: p.y-1}))} className="flex-1 bg-white border rounded">↑</button>
                  <button onClick={() => setSuitConfig(p => ({...p, y: p.y+1}))} className="flex-1 bg-white border rounded">↓</button>
                  <button onClick={() => setSuitConfig(p => ({...p, scale: p.scale+0.02}))} className="flex-1 bg-white border rounded">＋</button>
                  <button onClick={() => setSuitConfig(p => ({...p, scale: p.scale-0.02}))} className="flex-1 bg-white border rounded">－</button>
                </div>
              )}
            </div>
