...
const CLOTHES_DATA = {
  MALE: [1, 2, 3, 4, 5].map(i => ({
    id: `m${i}`,
    url: `${base}clothes/suit-m${i}.png`
  })),
  FEMALE: [1, 2, 3, 4, 5].map(i => ({
    id: `f${i}`,
    url: `${base}clothes/suit-f${i}.png`
  }))
};
...
const EzIDApp = () => {
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-gray-100 min-h-screen font-sans">
      <header className="text-center mb-4"><h1 className="text-blue-600 font-black text-2xl">EzID V3.21</h1></header>
      ...
      <div className="flex gap-2 overflow-x-auto mb-4 bg-white p-3 rounded-2xl border min-h-[70px]">
        ...
        {photoList.map((img, i) => (<img key={i} src={img} className="h-14 w-11 rounded border flex-shrink-0" />))}
      </div>
