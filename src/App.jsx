...
// V3.25 修改：回歸最原始的相對路徑，配合你已經移動到 public 的檔案
const CLOTHES_DATA = {
...
const EzIDApp = () => {
  ...
  const handleUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onload = (f) => {
        const img = new Image();
        img.onload = () => { setImage(img); setBgRemovedImage(null); setSelectedSuit(null); setPosX(0); setPosY(0); setScale(0.5); };
  ...
  const addToQueue = () => {
    ...
    if (selectedSuit) {
      ...
      sImg.src = selectedSuit.url; // 這裡會直接抓取 clothes/...
      sImg.onload = () => {
        tCtx.save();
      ...
      sImg.onerror = () => alert("衣服讀取錯誤，路徑可能是：" + selectedSuit.url);
    } else {
      setPhotoList(prev => [...prev, tempCanvas.toDataURL('image/png')]);
      setImage(null);
    }
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-gray-100 min-h-screen font-sans">
      <header className="text-center mb-4"><h1 className="text-blue-600 font-black text-2xl">EzID Taiwan V3.25</h1></header>
      
      {/* 介面部分保持不變 */}
      <div className="flex gap-1 mb-4 bg-gray-200 p-1 rounded-xl shadow-inner">
        {Object.values(SPECS).map(s => (
          <button key={s.id} onClick={() => {setCurrentSpec(s); setPhotoList([]);}} className={`flex-1 py-1.5 text-[10px] font-bold rounded-lg transition-all ${currentSpec.id === s.id ? 'bg-white text-blue-600 shadow-md' : 'text-gray-500'}`}>{s.label}</button>
        ))}
      </div>

      <div className="flex gap-2 overflow-x-auto mb-4 bg-white p-3 rounded-2xl border min-h-[70px] no-scrollbar">
        {photoList.length === 0 && <div className="text-gray-300 w-full text-center py-2 text-xs font-bold uppercase">Ready to Add</div>}
      ...
      {!image ? (
        <label className="block border-4 border-dashed border-gray-300 rounded-[2.5rem] p-16 text-center cursor-pointer bg-white">
          ...
          <span className="font-black text-gray-400 text-lg uppercase">Upload</span>
        </label>
      ) : (
        <div className="bg-white p-5 rounded-[2.5rem] shadow-2xl border space-y-4">
          <div className="relative w-full aspect-[35/45] rounded-3xl overflow-hidden bg-gray-200 shadow-inner">
            ...
            {isRemovingBg && <div className="absolute inset-0 bg-white/80 flex items-center justify-center font-bold">Processing...</div>}
          </div>
          ...
          <div className="space-y-4 bg-gray-50 p-4 rounded-3xl border">
            <div className="flex justify-between items-center">
              ...
              <button onClick={handleRemoveBg} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-lg">Remove BG</button>
            </div>

            <div className="border-t pt-4">
              <div className="flex gap-1 mb-2">
                <button onClick={() => setPosY(posY - 2)} className="bg-white border p-2 rounded flex-1 text-[10px] font-bold">UP</button>
                <button onClick={() => setPosY(posY + 2)} className="bg-white border p-2 rounded flex-1 text-[10px] font-bold">DOWN</button>
                <button onClick={() => setPosX(posX - 2)} className="bg-white border p-2 rounded flex-1 text-[10px] font-bold">LEFT</button>
                <button onClick={() => setPosX(posX + 2)} className="bg-white border p-2 rounded flex-1 text-[10px] font-bold">RIGHT</button>
              </div>
            ...
            <div className="border-t pt-4">
              <div className="flex gap-2 mb-3">
                <button onClick={() => setGender('MALE')} className={`flex-1 py-1.5 rounded-lg text-xs font-black ${gender === 'MALE' ? 'bg-blue-700 text-white' : 'bg-gray-200 text-gray-400'}`}>MALE</button>
                <button onClick={() => setGender('FEMALE')} className={`flex-1 py-1.5 rounded-lg text-xs font-black ${gender === 'FEMALE' ? 'bg-pink-600 text-white' : 'bg-gray-200 text-gray-400'}`}>FEMALE</button>
              </div>
              <div className="flex gap-2 overflow-x-auto py-1 mb-2 no-scrollbar">
                <button onClick={() => setSelectedSuit(null)} className="w-14 h-14 border-2 border-dashed rounded-xl flex-shrink-0 text-[10px] font-black text-gray-400 bg-white">NONE</button>
                {CLOTHES_DATA[gender].map(s => (
                  <button key={s.id} onClick={() => setSelectedSuit(s)} className={`w-14 h-14 border-2 rounded-xl flex-shrink-0 overflow-hidden ${selectedSuit?.id === s.id ? 'border-blue-500 bg-blue-50' : 'border-transparent bg-white shadow-sm'}`}>
              ...
              {selectedSuit && (
                <div className="grid grid-cols-4 gap-1">
                  <button onClick={() => setSuitY(suitY - 1)} className="bg-blue-50 text-blue-700 border border-blue-100 py-1.5 rounded-lg text-[9px] font-black">Suit ↑</button>
                  <button onClick={() => setSuitY(suitY + 1)} className="bg-blue-50 text-blue-700 border border-blue-100 py-1.5 rounded-lg text-[9px] font-black">Suit ↓</button>
                  <button onClick={() => setSuitScale(suitScale + 0.01)} className="bg-blue-50 text-blue-700 border border-blue-100 py-1.5 rounded-lg text-[9px] font-black">Size +</button>
                  <button onClick={() => setSuitScale(suitScale - 0.01)} className="bg-blue-50 text-blue-700 border border-blue-100 py-1.5 rounded-lg text-[9px] font-black">Size -</button>
                </div>
              )}
            </div>
          ...
          <div className="flex gap-3">
            <button onClick={() => setImage(null)} className="flex-1 bg-gray-200 py-4 rounded-3xl font-bold text-gray-500">Cancel</button>
            <button onClick={addToQueue} className="flex-[2] bg-blue-700 text-white py-4 rounded-3xl font-black text-lg shadow-xl active:scale-95 transition-transform">Add to Sheet</button>
          </div>
        </div>
      )}
      {photoList.length > 0 && !image && <button onClick={downloadPrint} className="w-full mt-4 bg-green-600 text-white py-5 rounded-3xl font-black text-xl shadow-2xl animate-pulse">Download 4x6 JPG</button>}
