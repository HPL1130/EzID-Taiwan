...
const EzIDApp = () => {
  ...
  // 沿用你 V3.0 原始的拼板邏輯 (此處簡化示意，請將 V3.0 downloadPrint 貼回即可)
  const downloadPrint = () => { /* ... 保持 V3.0 內容 ... */ };

  return (
    <div className="max-w-md mx-auto p-4 bg-gray-100 min-h-screen font-sans">
      <header className="text-center mb-4">
        <h1 className="text-blue-600 font-black text-2xl tracking-tighter">EzID V3.10</h1>
      </header>
        ...
        <div className="bg-white p-5 rounded-[2.5rem] shadow-2xl border space-y-4">
          ...
          <div className="space-y-4 bg-gray-50 p-4 rounded-3xl border">
            ...
            <div className="border-t pt-4">
              ...
              {/* 這裡就是你需要的調整按鈕，僅在選中衣服時顯示 */}
              {selectedSuit && (
                <div className="grid grid-cols-4 gap-2 mt-4 border-t pt-4">
                  <button onClick={() => setSuitY(suitY - 1)} className="bg-white border py-2 rounded-lg text-[10px] font-bold shadow-sm">上移</button>
                  <button onClick={() => setSuitY(suitY + 1)} className="bg-white border py-2 rounded-lg text-[10px] font-bold shadow-sm">下移</button>
                  <button onClick={() => setSuitScale(suitScale + 0.01)} className="bg-white border py-2 rounded-lg text-[10px] font-bold shadow-sm">放大</button>
                  <button onClick={() => setSuitScale(suitScale - 0.01)} className="bg-white border py-2 rounded-lg text-[10px] font-bold shadow-sm">縮小</button>
                </div>
              )}
            </div>
