...
const EzIDApp = () => {
    ...
    if (file) {
      ...
      reader.onload = (f) => {
        ...
        img.onload = () => { 
            ...
            setPosX(0); setPosY(0); setScale(0.5); // 重置位置
        };
  ...
  const handleRemoveBg = async () => {
    ...
    try {
      ...
      if (!remover) {
        throw new Error("去背模組尚未載入完成，請等待 5 秒並確保 index.html 正確。");
      }
    ...
    <div className="max-w-2xl mx-auto p-4 bg-gray-100 min-h-screen font-sans">
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
        {Object.values(SPECS).map(s => (
          <button key={s.id} onClick={() => {setCurrentSpec(s); setPhotoList([]);}} className={`px-4 py-2 text-sm font-bold rounded-xl whitespace-nowrap ${currentSpec.id === s.id ? 'bg-blue-600 text-white shadow-lg' : 'bg-white text-gray-500'}`}>{s.label}</button>
        ))}
      </div>

      {/* 預覽佇列 */}
      <div className="flex gap-2 overflow-x-auto mb-6 bg-white p-4 rounded-2xl border min-h-[80px]">
        {photoList.map((img, i) => (<img key={i} src={img} className="h-16 w-12 rounded border shadow-sm" />))}
      </div>

      {!image ? (
        <label className="block border-4 border-dashed border-gray-300 rounded-[2rem] p-16 text-center cursor-pointer bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-6 rounded-[2rem] shadow-xl">
          ...
          <div className="space-y-4">
            <div className="relative w-full aspect-[35/45] rounded-xl overflow-hidden bg-white border">
              ...
              {isRemovingBg && <div className="absolute inset-0 bg-blue-600/80 flex items-center justify-center font-bold text-white text-center p-4">AI 正在去背中...<br/>首次運行需下載模型(約30秒)</div>}
            </div>
            <div className="grid grid-cols-2 gap-2">
              <button onClick={handleRemoveBg} className="bg-purple-600 text-white p-4 rounded-xl font-bold active:scale-95 transition-transform">✨ 一鍵去背</button>
              <button onClick={addToQueue} className="bg-blue-600 text-white p-4 rounded-xl font-bold active:scale-95 transition-transform">✅ 確認加入</button>
            </div>
          </div>
            ...
            <div className="bg-gray-50 p-4 rounded-2xl space-y-4">
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  {BACKGROUND_COLORS.map(c => <button key={c.id} onClick={() => setSelectedBgColor(c.hex)} className={`w-8 h-8 rounded-full border-2 ${selectedBgColor === c.hex ? 'border-blue-500 scale-110' : 'border-white'}`} style={{backgroundColor: c.hex}} />)}
                </div>
              </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {['上','下','左','右'].map((d,i) => (
                    <button key={d} onClick={() => {
                      if(i===0) setPosY(prev => prev-5); if(i===1) setPosY(prev => prev+5);
                      if(i===2) setPosX(prev => prev-5); if(i===3) setPosX(prev => prev+5);
                    }} className="bg-white border p-3 rounded-xl font-bold active:bg-blue-100 transition-colors"> {d} </button>
                  ))}
                </div>
              ...
              <div className="space-y-4 border-t pt-4">
                ...
                <div className="flex gap-2 overflow-x-auto py-2">
                  <button onClick={()=>setSelectedSuit(null)} className="w-14 h-14 border-2 border-dashed rounded-xl flex-shrink-0 text-xs text-gray-400">無衣服</button>
                  {CLOTHES_DATA[gender].map(s => (
                    <img key={s.id} src={s.url} onClick={()=>setSelectedSuit(s)} className={`w-14 h-14 border-2 rounded-xl p-1 cursor-pointer ${selectedSuit?.id===s.id?'border-blue-600 bg-blue-50':'border-transparent bg-white'}`} />
                  ))}
                </div>
                {selectedSuit && (
                  <div className="grid grid-cols-3 gap-2">
                    {['衣上','衣下','衣大','衣左','衣右','衣小'].map((t,i) => (
                      <button key={t} onClick={() => {
                        if(i===0) setSuitY(prev => prev-1); if(i===1) setSuitY(prev => prev+1);
                        if(i===2) setSuitScale(prev => prev+0.02); if(i===3) setSuitX(prev => prev-1);
                        if(i===4) setSuitX(prev => prev+1); if(i===5) setSuitScale(prev => prev-0.02);
                      }} className="bg-white border border-blue-200 p-2 rounded-xl text-xs font-bold text-blue-600 active:bg-blue-600 active:text-white">{t}</button>
                    ))}
                  </div>
                )}
              </div>
            </div>
            <button onClick={()=>setImage(null)} className="w-full text-gray-400 text-sm font-bold">重新上傳</button>
          </div>
        </div>
      )}

      {photoList.length > 0 && !image && (
        <button onClick={downloadPrint} className="w-full mt-8 bg-green-600 text-white py-6 rounded-[2rem] font-black text-2xl shadow-2xl active:scale-95 transition-transform">💾 下載 4x6 拼板成品</button>
      )}
