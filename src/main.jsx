...
const EzIDApp = () => {
  ...
  const addToQueue = () => {
    ...
    if (selectedSuit) {
      ...
      sImg.onerror = () => alert("衣服圖片讀取失敗，請確認路徑。");
    } else {
      setPhotoList(prev => [...prev, tempCanvas.toDataURL('image/png')]);
      setImage(null);
    }
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-gray-100 min-h-screen font-sans text-gray-800">
      <header className="text-center mb-4"><h1 className="text-blue-700 font-black text-2xl">EzID 台灣證件照 V3.29</h1></header>
      ...
      {!image ? (
        ...
        <div className="bg-white p-5 rounded-[2.5rem] shadow-2xl border space-y-4">
          <div className="relative w-full aspect-[35/45] rounded-3xl overflow-hidden bg-gray-200 shadow-inner">
            ...
            {isRemovingBg && (
              <div className="absolute inset-0 bg-white/80 flex flex-col items-center justify-center">
                ...
                <span className="font-bold text-blue-600 text-xs">AI 去背中 (首次需載入模型)...</span>
              </div>
            )}
          </div>
            ...
            <div className="border-t pt-4">
              ...
              <div className="flex gap-2 overflow-x-auto py-1 mb-2 no-scrollbar">
                ...
                {CLOTHES_DATA[gender].map(s => (
                  <button key={s.id} onClick={() => setSelectedSuit(s)} className={`w-14 h-14 border-2 rounded-xl flex-shrink-0 overflow-hidden ${selectedSuit?.id === s.id ? 'border-blue-500 shadow-md' : 'border-transparent bg-white'}`}>
              ...
              {selectedSuit && (
                <div className="grid grid-cols-3 gap-1 mt-2">
                  <button onClick={() => setSuitY(suitY - 0.5)} className="bg-blue-50 text-blue-700 py-2 rounded-lg text-[10px] font-bold">衣上移</button>
                  <button onClick={() => setSuitY(suitY + 0.5)} className="bg-blue-50 text-blue-700 py-2 rounded-lg text-[10px] font-bold">衣下移</button>
                  <button onClick={() => setSuitScale(suitScale + 0.01)} className="bg-blue-50 text-blue-700 py-2 rounded-lg text-[10px] font-bold">衣放大</button>
                  <button onClick={() => setSuitX(suitX - 0.5)} className="bg-blue-50 text-blue-700 py-2 rounded-lg text-[10px] font-bold">衣左移</button>
                  <button onClick={() => setSuitX(suitX + 0.5)} className="bg-blue-50 text-blue-700 py-2 rounded-lg text-[10px] font-bold">衣右移</button>
                  <button onClick={() => setSuitScale(suitScale - 0.01)} className="bg-blue-50 text-blue-700 py-2 rounded-lg text-[10px] font-bold">衣縮小</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={addToQueue} className="flex-[2] bg-blue-700 text-white py-4 rounded-3xl font-black text-lg shadow-xl active:scale-95 transition-transform">確認加入</button>
          </div>
        </div>
      )}
      {photoList.length > 0 && !image && <button onClick={downloadPrint} className="w-full mt-4 bg-green-600 text-white py-5 rounded-3xl font-black text-xl shadow-2xl">下載 4x6 拼板</button>}
