  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-gray-100 min-h-screen">
      ...
      <div className="bg-white p-6 rounded-3xl shadow-lg">
        {!image ? (
          <label className="cursor-pointer block p-10 border-2 border-dashed rounded-xl text-center">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <button onClick={handleRemoveBg} className="bg-purple-600 text-white p-2 rounded flex-1 font-bold">
                {isRemovingBg ? "AI 去背中..." : "✨ 一鍵去背"}
              </button>
              <button onClick={addToQueue} className="bg-blue-600 text-white p-2 rounded flex-1 font-bold">✅ 加入排版</button>
            </div>
            {/* 規格與顏色 UI ... (保持原本完整版) */}
          </div>
        )}
